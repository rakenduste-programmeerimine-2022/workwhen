...
function axiosPost({ name, email, phone }, id, link) {
    return new Promise(async (resolve, reject) => {
        if (name !== "" || email !== "" || phone !== "" || id !== "") {
            axios
                .post(
                    `http://localhost:8080/contact/${link}`,
                    id !== ""
                        ? {
                                id,
                                name,
...
export default function Contacts() {
    ...
    const getData = () => {
        axios
            .get("http://localhost:8080/contact/all", { headers: { Authorization: `Bearer ${localStorage.getItem("token")}` } })
            .then(function (response) {
                setContacts([]);
            ...
            <Paper>
                    ...
                    <Table className="primaryTable">
                        ...
                        <TableHead variant="header" size="large" className="primary">
                            <TableRow>
                                ...
                                <TableCell>
                                    <Button onClick={handleOpenCnctAdd}>Add new contact</Button>
                                </TableCell>
                                <TableCell>
                                    <TextField label="search..." onChange={(e) => setSearchQuery(e.target.value)} />
                                </TableCell>
                            </TableRow>
                        </TableHead>
                        <TableBody className="primaryBody">
                            {contacts
                                ...
                                .filter((contacts) => {
                                    ...
                                    if (
                                        contacts.name.toLowerCase().toString().includes(searchQuery.toLowerCase().toString()) ||
                                        contacts.phone.toLowerCase().toString().includes(searchQuery.toLowerCase().toString()) ||
                                        contacts.email.toLowerCase().toString().includes(searchQuery.toLowerCase().toString())
                                    ) {
                                        return contacts;
                                    }
                                    return null;
                                ...
                                .map((contact, index) => (
                                    <TableRow key={index}>
                                        ...
                                        <TableCell className="contactButton">
                                            <Button onClick={handleOpenCnctChange} id={contact._id}>
                                                Edit
                                            </Button>
                                        </TableCell>
                                        <TableCell>
                                            <Button onClick={handleOpenCnctDelete} id={contact._id}>
                                                Delete
                                            </Button>
                                        </TableCell>
                                    </TableRow>
                ...
                >
                    ...
                    >
                        ...
                        <FormControl className="contactChangeForm" sx={{ width: "20rem", p: 2 }}></FormControl>
                        <TextField
                            id="cnctChange"
                            name="name"
                        ...
                        />
                        <DialogActions>
                            <Button variant="contained" margin="dense" onClick={handleCloseCnctChange}>
                                Cancel
                            </Button>
                            <Button variant="contained" onClick={handleContactChangeSave} autoFocus>
                                Change
                            </Button>
                        </DialogActions>
                    </Box>
                ...
                <Dialog
                    ...
                    sx={{}} // ?
                >
                    <DialogContentText id="alert-dialog-delete" sx={{ padding: "8% 0 0 5%" }}>
                        Are you sure?
                    </DialogContentText>
                    <DialogActions>
                        <Button
                ...
                >
                    ...
                    <FormControl className="contactAddForm" sx={{ width: "20rem", p: 2 }}></FormControl>
                    <TextField
                        autoFocus
                        id="cnctNameAdd"
                        label="New contact name"
                    ...
                    />
                    <DialogActions>
                        <Button variant="contained" margin="dense" onClick={handleCloseCnctAdd}>
                            Cancel
                        </Button>
                        <Button variant="contained" margin="dense" onClick={handleAddCnctSave}>
                            Save
                        </Button>
                    </DialogActions>
                </Dialog>
